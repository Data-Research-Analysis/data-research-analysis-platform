...
import sso_auth from './routes/sso_auth.js';
...
import meta_ads from './routes/meta_ads.js';
...
import marketing_metrics from './routes/marketing_metrics.js';
import campaign_analysis from './routes/campaign_analysis.js';
import campaign_targets from './routes/campaign_targets.js';
...
app.use(cors({
  origin: function (origin, callback) {
    ...
    if (allowedOrigins.indexOf(origin) !== -1 || origin.includes('localhost') || origin.includes('dataresearchanalysis.test')) {
      callback(null, true);
    } else {
...
app.use('/marketing-metrics', marketing_metrics);
app.use('/campaign-analysis', campaign_analysis);
app.use('/campaign-targets', campaign_targets);
